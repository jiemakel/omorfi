...
def analyse_tree(filename):
    ...
    if text.find('div'):
        ...
        for entry in entries:
            if entry.text.startswith("'flipa"):
                break
            ...
            if " " not in entry.text:
                ...
                pos = None
                if entry.text.endswith(" n.") or entry.text.endswith(" f.") or entry.text.endswith(" m."):
                    pos = "N"
                else:
                    x = ""
                    ...
                    if x in pos_replacements and x != "":
                        ...
                        pos = "V"
                    elif x.startswith("n.") or x.startswith("f.") or x.startswith("m."):
                        pos = "N"
                    elif x.startswith("adj.") or x.startswith("a."):
                        pos = "A"
                    elif x.startswith("adv."):
                        pos = "other"
                if pos is None:
                    pos = "UNK"
...
def write_to_file(filename, pos, entry):
    ...
    with open(filename + "_" + pos + ".uniq.freqs", "a", encoding="utf-8") as f:
        f.write("1 " + entry + "\n")
...
def clean_files(filename):
    for x in glob.glob(filename + "*.uniq.freqs"):
        print(x)
        os.remove(x)
